...
public:
    ...
    static std::unique_ptr<rpc::telemetry_server::SubakInfo> translateToRpcSubakInfo(const mavsdk::TelemetryServer::SubakInfo &subak_info)
    {
        auto rpc_obj = std::make_unique<rpc::telemetry_server::SubakInfo>();
    ...
    static mavsdk::TelemetryServer::SubakInfo translateFromRpcSubakInfo(const rpc::telemetry_server::SubakInfo& subak_info)
    {
        mavsdk::TelemetryServer::SubakInfo obj;
    ...
    grpc::Status PublishSubakInfo(
        grpc::ServerContext* /* context */,
        const rpc::telemetry_server::PublishSubakInfoRequest* request,
        rpc::telemetry_server::PublishSubakInfoResponse* response) override
    {
        if (_lazy_plugin.maybe_plugin() == nullptr) {
        ...
        if (request == nullptr) {
            LogWarn() << "PublishSubakInfo sent with a null request! Ignoring...";
            return grpc::Status::OK;
        }

        auto result = _lazy_plugin.maybe_plugin()->publish_subak_info(translateFromRpcSubakInfo(request->subak_info()));

        if (response != nullptr) {
            
            
            response->set_allocated_subak_info(translateToRpcSubakInfo(result).release());
