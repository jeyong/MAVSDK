...
public:
    ...
    /**
     * @brief SubakInfo message type.
     */
    struct SubakInfo {
        
        float x_value{float(NAN)}; /**< @brief */
        float y_value{float(NAN)}; /**< @brief */
    };

    /**
     * @brief Equal operator to compare two `Telemetry::SubakInfo` objects.
     *
     * @return `true` if items are equal.
     */
    friend bool operator==(const Telemetry::SubakInfo& lhs, const Telemetry::SubakInfo& rhs);

    /**
     * @brief Stream operator to print information about a `Telemetry::SubakInfo`.
     *
     * @return A reference to the stream.
     */
    friend std::ostream& operator<<(std::ostream& str, Telemetry::SubakInfo const& subak_info);
    ...
    */
    ...
    /**
    * @brief Callback type for subscribe_subak_info.
    */
        
    using SubakInfoCallback = std::function<void(SubakInfo)>;

    /**
     * @brief Subscribe to 'Subak Info' updates.
     */
    void subscribe_subak_info(SubakInfoCallback callback);



    /**
     * @brief Poll for 'SubakInfo' (blocking).
     *
     * @return One SubakInfo update.
     */
    SubakInfo subak_info() const;
